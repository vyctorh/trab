...
Among all wireless technologies WMN has advantages compared to single-hop infrastructure network architecture, because it communicates in multi-hop way that extends the coverage of the network and allowing communication with alternative paths in case of failures in links \cite{5622071,Fang2012}.


O desafio é adequar as WMN aos requisitos de comunicação requeridos pela AMI, onde centenas de medidores se comunicam com a  Central de Processamento através dos DAP (\emph{Data Aggregation Point}) que são os \emph{gateways} desta rede. A grande quantidade de nós é o principal desafio para uma WMN \cite{Akyildiz2005}, já que neste ambiente pode-se ter mais de 100 medidores associados a um DAP que podem simultaneamente enviar dados causando congestionamento na rede. Uma forma de mitigar este problema é o uso de múltiplos DAP. \cite{Gungor2011} cita problemas relacionados às condições de propagação do ambiente externo que proporcionam atenuações, interferências e variações, além de estarem sujeitas a ataques. 

O protocolo de roteamento tem que ser capaz de encontrar rotas confiáveis para não degradar a performance e poder atender aos requisitos da AMI. Os equipamentos de comunicação sem fio têm a capacidade de operar em diversas taxas de transmissão e são sensíveis às condições de propagação gerando outro desafio para o roteamento e para sua métrica. Esta última deve levar em consideração as oscilações que a qualidade dos enlaces sofre e a taxa de transmissão do enlace para uma avaliação coerente dos custos.


Neste trabalho dado os problemas enfrentados pelo roteamento em WMN para atender aos requisitos de comunicação da AMI, propomos um algoritmo de seleção dinâmica de DAP  para a comunicação entre medidores e a Central de Processamento (\emph{headend}). Neste problema, assume-se que cada medidor pode se conectar, através de múltiplos saltos, a um conjunto de DAP. O objetivo principal deste algoritmo, denominado \emph{DAP Dynamic Selection Algorithm} (DDSA), é aumentar a confiabilidade e a resiliência com a utilização de múltiplos DAPs pelos medidores melhorando com isso a performance em ambientes congestionados e com interferências, já que é possível a escolha de rotas menos congestionadas ou de outros DAP em caso de quebra de rota.


O restante desse texto está estruturado da seguinte forma. A Seção 2 descreve as particularidades da rede de dados da AMI e seus desafios. A Seção 3 apresenta trabalhos relacionados. A Seção 4 apresenta e explica o princípio de funcionamento do DDSA. A Seção 5 apresenta os resultados obtidos nas simulações. A Seção 6 conclui o artigo e apresenta ideias de trabalhos futuros.
